...
class Main {
    ...
    static int somaAbsoluta;

    static int somaAbsolutaCorrente;

    static Peca menorPecaIgnoravel;

    static int maximos[] = new int[1001];

    static Hashtable mapas[] = new Hashtable[1000];
        ...
        while (true) {
            ...
            for (int i = 0; i != pecaCount; i++) {
                ...
                if ((somaAbsoluta - ignoravel.cima - ignoravel.baixo) % 2 == 0) {
                    ...
                    // se ja encontrou
                    if (encontrado) {
                        // se a metade da peca menor for maior que a base dessa
                        // aqui, pode eskipar essa
                        if (menorPecaIgnoravel.cima + menorPecaIgnoravel.baixo < ignoravel.cima
                                + ignoravel.baixo) {
                            continue;
                        }
                    }
                    ...
                    //System.out.println("ignorando " + ignoravel + " :: " +
                    // somaAbsolutaCorrente);
                    //imprime();
                    tenta(0, 0, 0);
                }
    ...
    private static void imprime() {
        for (int i = 0; i != pecaCount; i++) {
            if (ignoravel != null && getPeca(i) == ignoravel)
                continue;
            Peca temp = getPeca(i);
            System.out.print("[" + temp.cima + "/" + temp.baixo + "/"
                    + temp.abs + "],");
        }
        System.out.println();
    }
    ...
    static boolean tenta(int p, int diferenca, int somaDeUmLado) {
        ...
        if (mapas[p].containsKey(new Integer(diferenca))) {
            return false;
        }

        // estou passando, se voltar nao venha aqui
        mapas[p].put(new Integer(diferenca),mut);
        ...
        if (tenta(p + 1, diferenca + posicaoAtual.abs, somaDeUmLado
                + posicaoAtual.cima)) {
            return true;
        }
...
class MyCompareThem {
    ...
    public int compare(Peca i1, Peca i2) {
        if (i1.abs > i2.abs) {
            return -1;
