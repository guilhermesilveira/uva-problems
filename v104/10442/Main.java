...
class Main10442 {
    ...
    private static BGG d16 = new BGG("16");
    ...
    private static final boolean run() throws IOException {
        ...
        for (; atual < len;) {
            ...
            if (cv == '#') {
                ...
                System.out.println("cortando: " + s.substring(pp, inicial - 1));
                BGG bgg = new BGG(s.substring(pp, inicial - 1), base);
                if(bgg.compareTo(d16) > 0) {
                    return false;
                }
                base = bgg.intValue();
                if(base<2) {
                ...
                System.out.println("nova base: " + base);
...
class BGG {
    ...
    public BGG(byte[] val) {
	if (val.length == 0)
	    throw new NumberFormatException("Zero length BGG");
    ...
    public BGG(String val) {
	this(val, 10);
    }

    private BGG(int[] magnitude, int signum) {
	this.signum = (magnitude.length==0 ? 0 : signum);
	this.mag = magnitude;
    }

    BGG(MutableBGG val, int sign) {
    ...
    private static BGG valueOf(long val) {
    ...
    private static BGG posConst[] = new BGG[MAX_CONSTANT+1];
    private static BGG negConst[] = new BGG[MAX_CONSTANT+1];
    static {
	for (int i = 1; i <= MAX_CONSTANT; i++) {
	    int[] magnitude = new int[1];
	    magnitude[0] = (int) i;
	    posConst[i] = new BGG(magnitude,  1);
	    negConst[i] = new BGG(magnitude, -1);
	}
    }

    public static final BGG ZERO = new BGG(new int[0], 0);

    public static final BGG ONE = valueOf(1);

    private BGG add(BGG val) {
        int[] resultMag;
	if (val.signum == 0)
            return this;
	if (signum == 0)
	...
	if (val.signum == signum)
            return new BGG(add(mag, val.mag), signum);
        ...
        return new BGG(resultMag, cmp*signum);
    }
    ...
    private BGG remainder(BGG val) {
        MutableBGG q = new MutableBGG(),
                          r = new MutableBGG(),
                          a = new MutableBGG(this.mag),
                          b = new MutableBGG(val.mag);

        a.divide(b, q, r);
        return new BGG(r, this.signum);
    }
    ...
    public BGG negate() {
	return new BGG(this.mag, -this.signum);
    }

    public BGG mod(BGG m) {
	if (m.signum <= 0)
	    throw new ArithmeticException("BGG: modulus not positive");

	BGG result = this.remainder(m);
	return (result.signum >= 0 ? result : result.add(m));
    }


    private BGG shiftLeft(int n) {
        if (signum == 0)
            return ZERO;
        if (n==0)
            return this;
        ...
        return new BGG(newMag, signum);
    }

    private BGG shiftRight(int n) {
        if (n==0)
            return this;
        if (n<0)
            return shiftLeft(-n);
    ...
    public BGG min(BGG val) {
	return (compareTo(val)<0 ? this : val);
    }

    public BGG max(BGG val) {
	return (compareTo(val)>0 ? this : val);
    }
	...
	int numGroups = 0;
	while (tmp.signum != 0) {
            BGG d = longRadix[radix];

            MutableBGG q = new MutableBGG(),
                              r = new MutableBGG(),
                              a = new MutableBGG(tmp.mag),
                              b = new MutableBGG(d.mag);
            a.divide(b, q, r);
            BGG q2 = new BGG(q, tmp.signum * d.signum);
            BGG r2 = new BGG(r, tmp.signum * d.signum);
    ...
    private static BGG longRadix[] = {null, null,
...
class MutableBGG {
	...
	private final int compare(MutableBGG b) {
	...
	private void copyValue(MutableBGG val) {
		int len = val.intLen;
	...
	private void rightShift(int n) {
		...
		this.intLen -= nInts;
		if (nBits == 0) return;
		int bitsInHighWord = BGG.bitLen(value[offset]);
	...
	void divideOneWord(int divisor, MutableBGG quotient) {
		...
		int shift = 32 - BGG.bitLen(divisor);
	...
	void divide(MutableBGG b, MutableBGG quotient,
			MutableBGG rem) {
		if (b.intLen == 0)
				throw new ArithmeticException("BGG divide by zero");
		// Dividend is zero
		if (intLen == 0) {
		...
		int shift = 32 - BGG.bitLen(d[0]);
		if (shift > 0) {
			// First shift will not grow array
			BGG.primitiveLeftShift(d, dlen, shift);
			// But this one might
			rem.leftShift(shift);
		}
