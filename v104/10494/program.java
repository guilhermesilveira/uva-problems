...
class Main2 {
	...
	public static void main(String args[]) throws Exception {
		...
		while ((line=readLine())!=null) {
		    ...
		    String s[] = split(line);
		    BigInteger2 b = new BigInteger2(s[0]);
		    BigInteger2 d = new BigInteger2(s[2]);
		    if(s[1].equals("/")) {
		        System.out.println(b.divide(d));
		    } else if(s[1].equals("%")) {
		        System.out.println(b.mod(d));
		    }
		}
...
class BigInteger {
    ...
    private static BigInteger2 valueOf(long val) {
    ...
    private static BigInteger2 posConst[] = new BigInteger2[MAX_CONSTANT+1];
    private static BigInteger2 negConst[] = new BigInteger2[MAX_CONSTANT+1];
    static {
	for (int i = 1; i <= MAX_CONSTANT; i++) {
	    int[] magnitude = new int[1];
	    magnitude[0] = (int) i;
	    posConst[i] = new BigInteger2(magnitude,  1);
	    negConst[i] = new BigInteger2(magnitude, -1);
	}
    }

    public static final BigInteger2 ZERO = new BigInteger2(new int[0], 0);

    public static final BigInteger2 ONE = valueOf(1);

    private static final BigInteger2 TWO = valueOf(2);

    public BigInteger2 add(BigInteger2 val) {
        int[] resultMag;
	if (val.signum == 0)
            return this;
	if (signum == 0)
	...
	if (val.signum == signum)
            return new BigInteger2(add(mag, val.mag), signum);
        ...
        return new BigInteger2(resultMag, cmp*signum);
    }
    ...
    public BigInteger2 subtract(BigInteger2 val) {
        int[] resultMag;
	if (val.signum == 0)
            return this;
	if (signum == 0)
    ...
    public BigInteger2 multiply(BigInteger2 val) {
        ...
        return new BigInteger2(result, signum*val.signum);
    }
    ...
    public BigInteger2 divide(BigInteger2 val) {
        ...
        a.divide(b, q, r);
        return new BigInteger2(q, this.signum * val.signum);
    }

    public BigInteger2 remainder(BigInteger2 val) {
        MutableBigInteger q = new MutableBigInteger(),
                          r = new MutableBigInteger(),
                          a = new MutableBigInteger(this.mag),
                          b = new MutableBigInteger(val.mag);
        ...
        a.divide(b, q, r);
        return new BigInteger2(r, this.signum);
    }
    ...
    public BigInteger2 negate() {
	return new BigInteger2(this.mag, -this.signum);
    }

    public BigInteger2 mod(BigInteger2 m) {
	if (m.signum <= 0)
	    throw new ArithmeticException("BigInteger: modulus not positive");

	BigInteger2 result = this.remainder(m);
	return (result.signum >= 0 ? result : result.add(m));
    }


    private BigInteger2 shiftLeft(int n) {
        if (signum == 0)
            return ZERO;
        if (n==0)
            return this;
        ...
        return new BigInteger2(newMag, signum);
    }

    private BigInteger2 shiftRight(int n) {
        if (n==0)
            return this;
        if (n<0)
            return shiftLeft(-n);
	...
	return compareTo((BigInteger2)o);
    }
	...
	if (!(x instanceof BigInteger2))
	    return false;
	BigInteger2 xInt = (BigInteger2) x;
    ...
    public BigInteger2 min(BigInteger2 val) {
	return (compareTo(val)<0 ? this : val);
    }

    public BigInteger2 max(BigInteger2 val) {
	return (compareTo(val)>0 ? this : val);
    }
	...
	BigInteger2 tmp = this.abs();
	int numGroups = 0;
	while (tmp.signum != 0) {
            BigInteger2 d = longRadix[radix];
            ...
            a.divide(b, q, r);
            BigInteger2 q2 = new BigInteger2(q, tmp.signum * d.signum);
            BigInteger2 r2 = new BigInteger2(r, tmp.signum * d.signum);
    ...
    private static BigInteger2 longRadix[] = {null, null,
...
class MutableBigInteger {
	...
	public String toString() {
		BigInteger2 b = new BigInteger2(this, 1);
		return b.toString();
	}
	private void rightShift(int n) {
		...
		this.intLen -= nInts;
		if (nBits == 0) return;
		int bitsInHighWord = BigInteger2.bitLen(value[offset]);
	...
	void divideOneWord(int divisor, MutableBigInteger quotient) {
		...
		int shift = 32 - BigInteger2.bitLen(divisor);
	...
	void divide(MutableBigInteger b, MutableBigInteger quotient,
		...
		int shift = 32 - BigInteger2.bitLen(d[0]);
		if (shift > 0) {
			// First shift will not grow array
			BigInteger2.primitiveLeftShift(d, dlen, shift);
			// But this one might
			rem.leftShift(shift);
		}
