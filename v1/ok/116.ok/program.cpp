...
void caminho() {
	...
	for(int i = 0; i < columns-2;i++) {
		cout << (anterior[menor][i]+1) << " ";
		menor = anterior[menor][i];
	}
	
	cout << (anterior[menor][columns-2]+1) <<endl;
	cout << menorSoma << endl;
}
...
int main() {
    ...
    while((cin >> rows >> columns)) {
         ...
         if(columns==1) {
         	menorSoma = TOP;
         	int minV;
            for(int i2=0;i2!=rows;i2++) if(matrix[i2][0]<menorSoma) { menorSoma = matrix[i2][0]; minV = i2;}
            cout << (minV+1) << endl;
            cout << menorSoma << endl;
         	continue;
         }
         ...
         for(int x=columns-2;x!=-1;x--) {
	         ...
	         for(int y=0;y!=rows;y++) {
			    ...
			    if(y == rows-1) {
			    	...
			    	int primeira = dp[0][x+1];
			    	int yMenos1 = dp[y-1][x+1];
			    	int yIgual = dp[y][x+1];
